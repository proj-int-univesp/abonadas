...
class ConsultaAbonadasDespacho(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        chefias_subordinadas = CargoChefia.objects.filter(subordinacao=self.request.user.funcionario.cargo_chefia)

        queryset = ReqAbonada.objects.filter(((Q(requerente__cargo_chefia__isnull=True) & 
                                            Q(requerente__lotacao__in=setores_chefia)) |
                                            (Q(requerente__cargo_chefia__isnull=False) &                                             
                                            Q(requerente__cargo_chefia__in=chefias_subordinadas))) &
                                            Q(situacao='T') &
                                            Q(data_abonada__gt=dt.now().date()))
...
class DespacharAbonada(LoginRequiredMixin, FormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if ((chefe.cargo_chefia == None) or 
            (requerente.cargo_chefia == None and requerente.lotacao.responsavel != chefe.cargo_chefia) or
            (requerente.cargo_chefia != None and requerente.cargo_chefia.subordinacao != chefe.cargo_chefia)):
...
class DetalhesAbonada(LoginRequiredMixin, DetailView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        # Verifica se o usuário é o requerente
        if ((usuario.cargo_comum is not None and requerente == usuario) or 
        # Verifica se o usuário é o chefe do requerente
            (usuario.cargo_chefia is not None and 
             (requerente.cargo_chefia is None and requerente.lotacao.responsavel == usuario.cargo_chefia) or
             (requerente.cargo_chefia is not None and requerente.cargo_chefia.subordinacao == usuario.cargo_chefia)) or
        # Verifica se o usuário é do setor de gestão de pessoas
